...
final signInAnonymouslyProvider = Provider.autoDispose<Future<void> Function()>(
  (ref) => () async {
    try {
      ...
      final appUser =
          await ref.read(baseAppUserRepositoryProvider).fetchAppUser(appUserId: user.uid);
      if (appUser == null) {
        await ref.read(baseAppUserRepositoryProvider).setAppUser(appUserId: user.uid);
      }
